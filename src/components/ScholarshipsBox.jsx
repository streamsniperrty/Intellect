...
function ScholarshipsBox() {
			...
			getDoc(docRef)
				.then((docSnap) => {
					if (docSnap.exists()) {
						...
						console.log("Array from firestore: " + userData.scholarships);
						setData(userData.scholarships);
						localStorage.setItem(
							"data_scholarships",
							userData.scholarships.join(","),
						);
						localStorage.setItem("dataAvail", true);
					} else {
						setDoc(docRef, { scholarships: [] })
							.then(() => {
								console.log("Scholarships array created!");
							})
							.catch((error) => {
	...
	useEffect(() => {
		if (localStorage.getItem("dataAvail")) {
			const data = localStorage.getItem("data_scholarships");
	...
	const updateData = (arg) => {
		const docRef = doc(db, "data", username);
		setDoc(docRef, { scholarships: [...arg] }, { merge: true })
			.then(() => {
